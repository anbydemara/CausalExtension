...
# 计算MMD损失
def mmd_loss(source_features, generated_features):
    # 高光谱图像块的MMD计算（可调整为合适的核函数）
    source_features = source_features.view(source_features.size(0), -1)  # 展平高光谱图像块
    generated_features = generated_features.view(generated_features.size(0), -1)
    xx = torch.mm(source_features, source_features.t())
    yy = torch.mm(generated_features, generated_features.t())
    xy = torch.mm(source_features, generated_features.t())
    rx = torch.diag(xx).unsqueeze(0).expand_as(xx)
    ry = torch.diag(yy).unsqueeze(0).expand_as(yy)
    K = torch.exp(- (rx.t() + rx - 2 * xx) / 102)
    L = torch.exp(- (ry.t() + ry - 2 * yy) / 102)
    P = torch.exp(- (rx.t() + ry - 2 * xy) / 102)
...
def train(epoch):
    ...
    for i, (data, label) in enumerate(loop):
        ...
        out_SD, feature_SD, domain_SD, re_loss_SD, mv_loss_SD, dif_loss_SD = model(data, domain_label_SD, alpha,
                                                                                   mode='train')
        out_ED, feature_ED, domain_ED, re_loss_ED, mv_loss_ED, dif_loss_ED = model(x_ED, domain_label_ED, alpha,
                                                                                   mode='train')
        #         out_ID, feature_ID, _, _, _, _ = model(x_ID, domain_label_ED, alpha, mode='train')

        #         zsrc = torch.cat([feature_SD.unsqueeze(1), feature_ED.unsqueeze(1)], dim=1)
        ...
        #         err_s_domain = loss_domain(domain_SD, domain_label)
        #   extend 域标签
        #         domain_loss_ED = loss_domain(domain_ED, domain_label_ED)
        #         err_t_domain = loss_domain(domain_TD, domain_label)

        #         err = err_t_domain + err_s_domain + cls_loss
        # 多视角

        out_TD, feature_TD, domain_TD, _, _, _ = model(x_TD, domain_label_SD, alpha, mode='train')

        cls_loss_TD = cls_criterion(out_TD, label_TD)  # TD分类损失

        cls_loss = cls_criterion(out_SD, label) + cls_criterion(out_ED, label)  # 分类损失

        domain_loss_TD = loss_domain(domain_TD, domain_label_ED)

        #         domain_loss = domain_loss_SD.mean() + domain_loss_ED.mean()
        #         err = domain_loss + cls_loss + MV_loss
        err = cls_loss + MV_loss * min(100, epoch) * 0.12
        ...
        # 对抗学习 -- 优化Generator
        #         num_adv = label.unique().size()
        #         zsrc_con = torch.cat([feature_TD.unsqueeze(1)], dim=1)
        #         con_loss_adv = 0
        # #         idx_1 = np.random.randint(0, zsrc.size(1))

        #         for j, id in enumerate(label.unique()):    # 按类别遍历
        #             mask = label == label.unique()[j]
        #             z_SD_i, zsrc_i = feature_SD[mask], zsrc_con[mask]    # 筛选第j类的SD和&
        #             y_i = torch.cat([torch.zeros(z_SD_i.shape[0]), torch.ones(zsrc_i.shape[0])])
        #             zall = torch.cat([z_SD_i.unsqueeze(1).detach(), zsrc_i], dim=0)
        #             if y_i.size()[0] > 2:
        #                 con_loss_adv += con_criterion(zall, y_i)
        #         con_loss_adv = con_loss_adv / label.unique().shape[0]
        loss = cls_loss_TD + domain_loss_TD

        mmd = mmd_loss(data, x_TD)
        if mmd >= 0.5:
            print(mmd)
            loss += mmd  # 超出阈值则增加MMD损失
        ...
        #         writer.add_scalar('con_adv_loss', con_loss_adv, epoch)
...
if __name__ == '__main__':
    ...
    M_opt = Adam(model.parameters(), lr=args.lr)
