...
def train(epoch):
    ...
    for i, (data, label) in enumerate(loop):
        ...
        out_SD, feature_SD, domain_SD, re_loss_SD, mv_loss_SD, dif_loss_SD = model(data, domain_label_SD, alpha, mode='train')
        out_ED, feature_ED, domain_ED, re_loss_ED, mv_loss_ED, dif_loss_ED = model(x_ED, domain_label_ED, alpha, mode='train')
...
#         zsrc = torch.cat([feature_SD.unsqueeze(1), feature_ED.unsqueeze(1)], dim=1)
        ...
        MV_loss = (mv_recon_loss + mv_cls_loss + 0.1 * mv_dis_loss) * 0.1
...
#         err = err_t_domain + err_s_domain + cls_loss
        ...
        out_TD, feature_TD, domain_TD, _, _, _ = model(x_TD, domain_label_ED, alpha, mode='train')
