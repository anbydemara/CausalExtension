...
def train(epoch):
    ...
    for i, (data, label) in enumerate(loop):
        ...
        out_SD, domain_SD, band_SD, feat_SD = model(data, alpha, mode='train')
        out_ED, domain_TD, band_ED, feat_ED = model(x_ED, alpha, mode='train')

        # source 域标签
        domain_label = torch.zeros(out_SD.shape[0]).long().to(args.gpu)
        err_s_domain = loss_domain(domain_SD, domain_label)

        # extend 域标签
        domain_label = torch.ones(out_ED.shape[0]).long().to(args.gpu)
        err_t_domain = loss_domain(domain_TD, domain_label)


        assert feat_SD.size(0) == feat_ED.size(0)
        out_TD = model(x_TD, None)
