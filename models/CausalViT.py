...
from models.transformer_block import Block_ours, get_sinusoid_encoding, Block
...
def T2t_vit_7_feature(pretrained=False, **kwargs): # adopt performer for tokens to token
    model = T2T_ViT_Feature(img_size=13, in_chans=102, tokens_type='performer', embed_dim=256, depth=4, num_heads=4, mlp_ratio=2., **kwargs)
...
if __name__ == '__main__':
    model = T2t_vit_7_feature(num_classes=7, final_k=4)
    classif = classifier(num_classes=7)
    data = torch.randn(1, 102, 13, 13)
    x_causal, x_spurious, x_mix = model(data)
    print(classif(x_causal).shape)